...
const publisherSignals = [
  ...
  ["inherited reusable-workflow secrets", /^\s*secrets\s*:\s*["']?inherit["']?\s*$/im],
  ["Docker registry login action", /\buses\s*:\s*["']?docker\/login-action@/i],
  ["Docker publishing action", /\buses\s*:\s*["']?docker\/build-push-action@/i],
  [
    "registry attestation upload",
    /^\s*push-to-registry\s*:\s*["']?true["']?\s*(?:#.*)?$/im,
  ],
  ["publishing action input", /^\s*push\s*:\s*["']?true["']?\s*(?:#.*)?$/im],
  ...
  ["alternate container push command", /\b(?:podman|buildah|nerdctl)\b[^\r\n]*\bpush\b/i],
...
const adversarialFixtures = [
  ...
  ["contents write permission", "jobs:\n  release:\n    permissions: { contents: write }"],
  ["package write permission", "jobs:\n  release:\n    permissions:\n      packages: write"],
  ["attestation write permission", "jobs:\n  release:\n    permissions: { attestations: write }"],
  ["OIDC write permission", "jobs:\n  release:\n    permissions:\n      id-token: write"],
  ["secret-backed credential", "env:\n  TOKEN: ${{ secrets.REGISTRY_TOKEN }}"],
  ["inherited reusable-workflow secrets", "jobs:\n  release:\n    secrets: inherit"],
  ["Docker registry login action", "steps:\n  - uses: docker/login-action@deadbeef"],
  ["Docker publishing action", "steps:\n  - uses: docker/build-push-action@deadbeef"],
  ["registry attestation upload", "with:\n  push-to-registry: true"],
  ["publishing action input", "with:\n  push: true"],
  ["Docker push command", "run: docker --config /tmp/config image push ghcr.io/x/app:sha"],
  ["Docker registry login command", "run: docker --config /tmp/config login ghcr.io"],
  ["Docker build push flag", "run: docker buildx build --push -t ghcr.io/x/app:sha ."],
  ["OCI publishing tool", "run: crane append --base source --new_layer layer"],
  ["alternate container push command", "run: buildah push app ghcr.io/x/app:sha"],
  [
    "outbound reusable workflow",
