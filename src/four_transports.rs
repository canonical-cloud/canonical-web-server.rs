#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `canonical-api-server`.
//!
...
//! to canonical-cloud — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `canonical-lib`, read-only |
//! | 2 | `http` | stateless request to the `canonical-api-server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `canonical-api-server` cluster |
...
//! `canonical-lib`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `CANONICAL_API_URL`, `CANONICAL_NATS_URL`, and the rest; see the
/// table in `ores-transport`'s README.
pub const ENV_PREFIX: &str = "CANONICAL";
...
pub const SERVICE_SLUG: &str = "canonical";
