//! Four-avenue web ↔ API binding for `canonical-cloud`.
//!
//! 1. Direct read-only DB via `*-lib-core` named queries (no migrations).
//! 2. Stateless HTTP from `app.canonical.cloud` to `api.canonical.cloud`.
//! 3. Stateful TLS 1.3/mTLS TCP to `api.canonical.cloud:7443`.
...
pub const GITHUB_ORG: &str = "canonical-cloud";
pub const ORG_SLUG: &str = "canonical-cloud";
pub const DNS_ZONE: &str = "canonical.cloud";
