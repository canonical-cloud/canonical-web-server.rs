...
Secrets for `canonical-cloud/canonical-web-server.rs` are **committed, encrypted**, with [sops] + [age],
...
- **No multi-line values.** A PEM must be represented on one dotenv line with escaped `\n` separators. Use a non-secret placeholder such as:
  `JWT_PRIVATE_KEY="<PEM-private-key-with-escaped-newlines>"`
...
```sh
just env-docker-run dev ghcr.io/canonical-cloud/canonical-web-server.rs:dev
