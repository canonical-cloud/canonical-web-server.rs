...
const WEB_CONTRACT: &str = include_str!("../../../.cli-flags.toml");
const SESSION_REVOKER_CONTRACT: &str = include_str!("../../../.cli-flags.revoker.toml");

/// Selects the independently audited configuration surface for a process.
#[derive(Clone, Copy)]
pub enum Contract {
    /// Customer-facing web server.
    Web,
    /// No-ingress session revocation worker.
    SessionRevoker,
}

fn contract(kind: Contract) -> &'static str {
    match kind {
        Contract::Web => WEB_CONTRACT,
        Contract::SessionRevoker => SESSION_REVOKER_CONTRACT,
    }
}
...
pub fn process_control(
    ...
    binary_name: &str,
    version: &str,
    known_command: Option<&str>,
) -> Result<Option<String>, String> {
    let values = try_resolved(kind, known_command)?;
    if values.get("CLI_HELP_REQUESTED").map(String::as_str) == Some("true") {
        return help_table(contract(kind), binary_name).map(Some);
    }
...
fn try_resolved(
    kind: Contract,
    known_command: Option<&str>,
) -> Result<&'static BTreeMap<String, String>, String> {
    RESOLVED
        .get_or_init(|| resolve(kind, known_command))
        .as_ref()
        .map_err(Clone::clone)
}
...
fn resolve(
    ...
    let argv = argv_without_command(std::env::args().collect(), known_command);
    resolve_from(contract(kind), &argv, std::env::vars())
}

fn argv_without_command(mut argv: Vec<String>, known_command: Option<&str>) -> Vec<String> {
    if known_command.is_some_and(|command| argv.get(1).map(String::as_str) == Some(command)) {
        argv.remove(1);
    }
    argv
}
...
fn resolve_from(
    ...
    contract
        .write_all(contract_text.as_bytes())
    ...
    if !resolved.contains_key("CANONICAL_INTERNAL_AUTH_TOKEN") {
        if let Some(value) = resolved.get("CANONICAL_WEB_SERVICE_TOKEN").cloned() {
            resolved.insert("CANONICAL_INTERNAL_AUTH_TOKEN".to_owned(), value);
        }
    }
...
fn help_table(contract_text: &str, binary_name: &str) -> Result<String, String> {
...
mod tests {
    ...
    fn command_line_overrides_environment_with_a_typed_value() {
        let contract = WEB_CONTRACT.parse::<toml::Value>().expect("contract");
    ...
    fn help_is_derived_from_the_audited_contract() {
        let help = help_table(WEB_CONTRACT, "canonical-web-server").expect("help");
        assert!(help.contains("--help"));
        assert!(help.contains("Options:"));
        assert!(help.starts_with("Usage: canonical-web-server"));
    }

    #[test]
    fn revoker_help_is_derived_from_its_separate_contract() {
        let help = help_table(
            contract(Contract::SessionRevoker),
            "canonical-session-revoker",
        )
        .expect("help");
        assert!(help.contains("--database-max-connections"));
        assert!(!help.contains("--app-base-url"));
    }

    #[test]
    fn known_subcommand_is_removed_without_consuming_flags() {
        assert_eq!(
            argv_without_command(
                vec![
            ...
            vec!["canonical-web-server", "--port=9000"]
        );
        assert_eq!(
            argv_without_command(
                vec!["canonical-web-server".to_owned(), "--help".to_owned()],
                None,
            ),
            vec!["canonical-web-server", "--help"]
        );
    }
}
