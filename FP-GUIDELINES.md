...
check was introduced: **288 findings across 63 files
and 13,614 lines**. CI compares against that budget and fails only when a
...
|---|---:|---|---|---|
| `RS003` | 133 | error | typed errors | panic-based control flow (`unwrap`/`expect`/`panic!`) |
| `RS001` | 60 | warn | immutable values | mutable local binding (`let mut`) |
| `TS006` | 28 | warn | typed errors | `throw` as control flow |
| `XX002` | 17 | warn | explicit outputs | long function body |
...
*typed errors* · 133 occurrences at baseline
...
*immutable values* · 60 occurrences at baseline
