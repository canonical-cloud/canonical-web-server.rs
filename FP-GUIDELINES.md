...
check was introduced: **325 findings across 64 files
and 14,504 lines**. CI compares against that budget and fails only when a
...
|---|---:|---|---|---|
| `RS003` | 160 | error | typed errors | panic-based control flow (`unwrap`/`expect`/`panic!`) |
| `RS001` | 68 | warn | immutable values | mutable local binding (`let mut`) |
| `TS006` | 28 | warn | typed errors | `throw` as control flow |
| `XX002` | 18 | warn | explicit outputs | long function body |
| `RS004` | 13 | warn | illegal states excluded by types | wildcard match arm defeats exhaustiveness |
| `TS002` | 11 | warn | immutable values | mutable `let` binding |
| `XX001` | 8 | warn | composition | oversized module |
| `TS004` | 7 | warn | pure transformations | in-place array mutation |
| `TS009` | 6 | warn | explicit inputs | ambient impurity (clock / randomness / env) read inside pure code |
| `RS005` | 4 | warn | typed errors | untyped/erased error in a signature |
| `RS008` | 2 | warn | explicit outputs | `&mut self` method returning unit |

## How to clear the top offenders

### `RS003` — panic-based control flow (`unwrap`/`expect`/`panic!`)

*typed errors* · 160 occurrences at baseline
...
*immutable values* · 68 occurrences at baseline
...
*typed errors* · 28 occurrences at baseline
...
*explicit outputs* · 18 occurrences at baseline
...
*composition* · 8 occurrences at baseline
...
*explicit inputs* · 6 occurrences at baseline
